...
async function loadAlbumUsageById(): Promise<Map<string, { imageCount: number, bytesUsed: number }>> {
  ...
  const rows = await db
    .select({
      ...
      bytesUsed: sql<number>`COALESCE(SUM(${imagesTable.bytes} + COALESCE(${imagesTable.thumbnailBytes}, 0) + COALESCE(${imagesTable.originalBytes}, 0)), 0)`,
    })
    .from(imagesTable)
...
async function buildStorageMetaInternal(): Promise<StorageMeta> {
  ...
  const [imageTotals] = await db
    .select({
      ...
      totalBytesUsed: sql<number>`COALESCE(SUM(${imagesTable.bytes} + COALESCE(${imagesTable.thumbnailBytes}, 0) + COALESCE(${imagesTable.originalBytes}, 0)), 0)`,
