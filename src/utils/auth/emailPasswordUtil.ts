...
function validateField(value: string, rules: ValidationRule[], setError: (value: boolean) => void): boolean {
...
function emailPasswordUtil(email: string, password: string, setEmailError: (value: boolean) => void, setPasswordError: (value: boolean) => void): boolean {
  let hasError = false

  const emailRules: ValidationRule[] = [
    { validate: email => email.trim() !== '', message: '邮箱不能为空' },
    {
      validate: email => /\S[^\s@]*@\S+\.\S+/.test(email),
      message: '邮箱格式不正确',
    },
    {
